...
namespace JdSuite.DataFilter.Xml
{
    public class XPathExtensionFunctions : System.Xml.Xsl.IXsltContextFunction
    {
        ...
        private bool EndsWith(string data,string endPattern)
        {
            return data.EndsWith(endPattern);
        }
        ...
        {
            if(FunctionName=="ends-with")
            {
                return EndsWith(Convert.ToString(args[0]), Convert.ToString(args[1]));
            }
